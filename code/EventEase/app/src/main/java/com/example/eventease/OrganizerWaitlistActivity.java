...
public class OrganizerWaitlistActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // ---Starting the Auth and Data Loading Process ---
        signInAndLoadData();
    }
    ...
    private void signInAndLoadData() {
        ...
        if (currentUser != null) {
            ...
            // Log the error and inform the user. Do not attempt a fake login.
            Log.e(TAG, "CRITICAL: No user is signed in. Cannot load data for this screen.");
            Toast.makeText(this, "Authentication Error. Please sign in again.", Toast.LENGTH_LONG).show();

            // Set the UI to an error state.
            eventNameTextView.setText("Authentication Error");
    ...
    private void deleteEventFromFirestore() {
        ...
        // Sending the delete request to Firestore
        db.collection("events").document(currentEventId)
                .delete()
                .addOnSuccessListener(aVoid -> {
                    // Success!
                    ...
                    // Closing this activity and go back to the previous screen
                    finish();
                })
                .addOnFailureListener(e -> {
                    // Failure!
